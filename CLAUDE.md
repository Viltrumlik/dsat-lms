...
*Oxirgi yangilangan: Phases 1–2 COMPLETE (merged to main). Phase 3 in progress (branch `feat/phase3-admin`): **3A — admin user management SHIPPED** (+ post-audit hardening), **3B — content studio SHIPPED** (question authoring + review lifecycle + §9 versioning; backend 30 tests + frontend editor with KaTeX preview). Next: **3C — exam builder + assignments** (`apps/assessments` — ExamTemplate/Section/Question/Assignment; assembles published questions). Backend-first, endpoints under /api/v1/admin/.*
