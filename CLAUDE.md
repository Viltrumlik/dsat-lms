...
> **Status:** plan LOCKED. **MVP COMPLETE — S0 + S1 + S2 + S4 shipped (+ S3 fast-follow pulled forward)** (branch `feat/phase4-teacher-crm`). **S0**: `apps/support`
...
> the rec to ACTED. All verified end-to-end in the browser + Playwright e2e. **MVP done; next up: fast-follows
> S5 (Office Hours), S6 (Mentor), S7 (Admin ops).** Full design doc lived in a session scratchpad; this section
...
| **S6** | Academic Mentor (in `apps/academy`): mentor FK + `MentorAssignment`/`MentorCheckIn`/`ParentContactLog` + new permission classes; auto-close on lifecycle change. | academy CRM |
| **S7** | Admin ops dashboard: KPI endpoints + optional `SupportOpsDaily` beat + `(admin)/admin/support-ops`. | S1, S2, S4, S5 |
