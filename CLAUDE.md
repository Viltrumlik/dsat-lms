...
***Classroom — one class page, both roles** (commits 7d7be2f backend, a0105f6 frontend). The stream was all a class had, and each role reached it through its own screen: the student via `/classes/[id]`, the teacher bolted underneath their roster. Google Classroom has **one** class page; so does this now.

**Tabs: Stream · Classwork · People**, rendered by a single `ClassWorkspace` that `/classes/[id]` and `/teacher/classes/[id]` both mount. What differs comes from the server, not from the file: `GET /classes/{id}/` publishes `my_role` + a **`capabilities`** object (`can_post` · `can_moderate` · `can_manage_roster` · `can_grade` · `can_see_submissions`) derived once in `views_classroom._capabilities`. `ClassStream` no longer compares `user.role` against a `STAFF_ROLES` list — it reads the capability, so the button and the permission cannot drift apart (a test asserts the pair: `can_post: false` **and** the POST really 403s).

**Classwork merges homework with the stream's `material` posts**, because a student should not have to remember which one a thing was posted as. Each role gets the half that is theirs: a student their own `my_status` (and `submitted_count: null`), staff the hand-in count (and `my_status: null`); unpublished homework is staff-only. A homework row leads where that role can act — `/homework/{id}` to hand in, `/teacher/homework/{id}` to mark.

**People** shows a student their classmates by name, as Classroom does — **`email` is nulled for a non-staff viewer**, and the teacher's `/teacher/classes/{id}/roster/` keeps carrying the enrolment state this deliberately does not. Staff additionally get the enrol-by-email form, the one management action that belongs on the class page. `components/teacher/ClassRoster.tsx` is **deleted** — the People tab is what it was, plus classmates.

**`GET /classes/` now returns classes you TEACH as well as ones you attend** (previously enrolled-only). Being in a class is a different job from administering one; `/teacher/classes/` stays the management list (create a class), and one membership list means one way into the workspace whichever role you hold.

Access is unchanged and shared with the stream — `_member_class_or_404`: class teacher, actively enrolled student, or full-access staff; everyone else **404, not 403**.

**Verified:** 853 pytest (13 new — teacher-in-own-class, capabilities-match-enforcement, classmate emails nulled, the my_status/submitted_count split, unpublished staff-only, materials folded in, cross-class non-leak, outsider 404) + ruff/black; frontend type-check + lint + 137 vitest + `next build`; **browser-verified both roles on the same class** (teacher: composer + "1 handed in" + emails + enrol form; student: no composer, own Graded/Submitted badges, classmates without addresses, rows linking to `/homework/`) and the tabs in **EN + UZ**. Also fixed "1 replies" → `replyCountOne`/`replyCountOther` via `plural()`.*

***Full mock — four modules, a break, and full screen** (commits 9f25850 backend, bcf955b frontend). Mocks were startable but not really *sittable*: one section, no break, and nothing stopping a student tabbing away mid-paper.

**The paper.** `seed_full_mock` builds it — **Math 1 (35 min · 22q) · Math 2 (35 · 22) · 10-minute break · English 1 (32 · 27) · English 2 (32 · 27)**, no question used twice. (The official Digital SAT runs R&W first and Math after the break; the order is the `MODULES` list in the command and is one edit either way.) Two new fields: `ExamSection.break_after_minutes` puts the rest on the module it **follows**, so the structure is the paper's rather than a rule the runner infers from how many modules there are; `ExamTemplate.requires_fullscreen` marks a paper invigilated, back-filled by the migration onto the types that already refuse a pause (mock · midterm · assessment · past_paper) — a clock you may not stop is a paper you sit in full screen.

**Why the mock carries NO whole-paper `time_limit`.** That is what makes the break free. `exam_time_remaining` runs from `started_at` wherever the student is, so ten minutes of rest under a paper-wide limit would come straight out of the exam. Only the per-module clocks bind, and each starts server-side when the advance PATCH lands — so resting *longer* delays the next module but never shortens it (pinned by `test_full_mock.py`).

**Full screen.** A browser grants it only from a **user gesture**, and that single fact shapes the whole design: the paper waits behind a **Begin screen** (that click is the gesture), and leaving raises a blocking overlay whose one control asks again. `lib/hooks/useFullscreen.ts` owns it. The rule that matters is `shouldBlock` — a pure function, unit-tested — which judges on having **LEFT** (`everEntered && !isFullscreen`), never on the request being *refused*: an embedded frame, a kiosk or an iPhone would otherwise wall a student out of their own exam behind a button that can never work. Exits are counted and shown back.

**The break** is a real countdown that auto-advances (`BreakScreen` reads `breakAfterMinutes` off the finished module), with "Resume testing" to end it early. It is deliberately a **display**, not a server rule — it does not need to be one, given the module clocks above.

**Gotcha the browser pass caught:** the seed originally titled sections "Module 1: Math", which rendered as "Section 1, Module 1: Module 1: Math" — `examLabels.sectionLabel` derives the numbering itself and treats `section.title` as the **subject half**. Section titles are subjects only.

**Verified:** 863 pytest (10 new — break-on-the-right-module, four separate clocks, no paper-wide clock, pause refused, next-module clock after advance, seed shape + idempotency + no reused questions) + ruff/black/makemigrations; frontend type-check + lint + **142 vitest** (5 new `useFullscreen.test.ts`) + `next build`; **browser-verified end to end** — Begin gate → "Section 1, Module 2: Math" with its own 35-min clock → Finish section → "Take a break" counting 10:00 down → Resume → "Section 2, Module 1: English" on a fresh 32-min clock; Begin screen checked in **EN + UZ**.

**Known limits (unchanged by this work, worth stating):** the session payload still ships every module's questions up front, so the break is not a barrier to reading ahead in devtools; and the break countdown, being client-side, bounds nothing for a student who reloads. Both need lazy per-module serving to fix properly.*

***One module at a time** (commit 2142e3c). Closing the leak the full-mock work left open: the session payload used to carry **every** module's questions at start, so the ten-minute break was a window in which the rest of the paper was already sitting in the tab, readable in devtools by anyone who thought to look.

`SessionSectionSerializer` now fills `questions` **only for `obj.current_section`**; every other module carries shape alone — `question_count`, `time_limit`, `break_after_minutes`, title, module — which is everything the runner needs to number modules, size the break screen and count the paper, and nothing to read ahead with. Safe precisely because sections are **forward-only**: a module is fetched exactly once, when it opens, and the advance PATCH that stamps its clock is the same response that hands it over (`BreakScreen` merges it via the new `sessionStore.loadSections`, which keeps questions it already holds so a closed module coming back empty doesn't wipe them). A mid-paper reload gets only the module in hand. Payload for the 86-question mock drops ~60 KB → ~19 KB.

**Two knock-ons, both fixes in their own right:**
- **Counting moved onto `questionCount`** (`TestShell` totals, `BottomBar`'s "Question X of Y", `BreakScreen`'s up-next, `ReviewScreen`'s answered-of). Summing the question *arrays* would report an 86-question mock as one module long — the browser pass caught exactly this as "0 savol" on the break screen. `answeredCount` now counts `questionStates` directly rather than walking sections.
...
***Ready for a server — deployment, and the optimizations measuring it found** (commits a82ea5f deploy, 7dc1760 perf).

**There was no way to run this on a server.** A dev Dockerfile, no production compose, no proxy config, no WSGI config, nothing for a load balancer to ask. Added:

- **`backend/Dockerfile` · `frontend/Dockerfile`** — multi-stage so the build toolchain never reaches the runtime, non-root (uid 10001), static **collected at build time** (an image that must reach the network to become servable fails to start on a bad day). Next.js moves to `output: 'standalone'` + `poweredByHeader: false`.
- **`backend/gunicorn.conf.py`** — sync workers (the slow work is all on Celery; async costs the per-request connection `CONN_MAX_AGE` depends on), 30s timeout, `max_requests` + jitter, `keepalive` **above** the proxy's (below it and Nginx reuses a socket gunicorn just closed → 502 from nowhere), probe lines filtered out of the access log.
- **`docker-compose.prod.yml`** — only Nginx publishes a port. **`migrate` is its own unit**, never the web container's command: two replicas both migrating is a race, and a failed migration should stop the deploy rather than crash-loop a container that was serving. Redis is cache *and* broker with `maxmemory-policy noeviction`, so a full instance refuses writes instead of silently dropping a queued email. Beat must stay at **one replica**.
- **`deploy/nginx.conf` + `proxy_params.conf`** — TLS, gzip once at the edge, `immutable` caching for hashed assets, rate limits in front of the auth endpoints. **X-Forwarded-For is appended (`$proxy_add_x_forwarded_for`), not set** — overwriting it would let a client forge the value the throttles key on. Pair with `NUM_PROXIES=1`.
- **`/healthz` + `/readyz`** (`common/health.py`) — liveness touches **nothing** (a database blip must not restart every web container) and readiness 503s so an instance leaves the pool without being killed. The cache check **round-trips**: a cache that accepts writes and returns nothing passes a `ping`.
- **WhiteNoise finally in MIDDLEWARE** (it was in requirements, unused, so the admin and DRF's browsable API had no styles) at position 1, with `CompressedManifestStaticFilesStorage`.
- **`check --deploy` is silent and therefore usable as a gate.** Zero security warnings; the ~230 drf-spectacular ones that buried them are silenced by code with a comment — a gate nobody reads is not a gate.
- `deploy/README.md` (runbook, what must be set, scaling knobs) + `.env.example` extended.

**Optimizations, measured not guessed.** A query-count sweep over every hot endpoint found the list views already clean (3–8 queries) — except one: **the class Classwork tab at 46**, from an `attachments.count()` per row that I had written two turns earlier. Annotated → **5**, and pinned by a test asserting ten homeworks cost the same as one. Then:

- **Two indexes.** `ExamResponse(question, -answered_at)` — `attempt_annotations` runs a correlated subquery per row of every bank page, and the unique constraint leads on `session_id` so it could not serve a lookup by question; without it the bank slows with every answer anyone ever gives. `ExamSession(user, exam, status)` — `live_session_for` and the attempt cap both ask exactly that, and only `(user, status)` existed.
- **Taxonomy cached in Redis** (`question_bank/cache.py`). The right thing to cache and nearly the only one: everything else worth serving is per-student, i.e. a key per user, i.e. a cache that never hits. Invalidated by **version bump** (one atomic write; deleting derived keys races a reader into repopulating a stale entry) fired from a **model signal** (the admin views are not the only writer — `seed_sat_taxonomy`, the Django admin and a shell all write these models). **Fails open** — and a test caught that the first version 500d instead, because the key derivation reads the version out of Redis and sat outside the guard.
- **`CONN_HEALTH_CHECKS = True`** — without it the first request after a database restart or failover is handed a dead socket from the pool. One free 500 per worker, every time.

**Verified:** 902 pytest (7 health + 7 taxonomy-cache + 1 query-count regression) + ruff/black/makemigrations; `check --deploy` clean under production settings; `collectstatic` green through the WhiteNoise manifest storage with the middleware at position 1; `next build` produces `.next/standalone/server.js`; frontend type-check + lint + 146 vitest. **Not verified: the Docker images have not been built** — Docker is not available on this machine, so the Dockerfiles and compose file are reviewed and internally consistent but unexercised. Build them once before trusting a deploy.*

***The audit, and everything it found* (commits 29ff8d5 · 6da25b3 · a5dcddd · 959c1f4 · caba262 · 6ac0fbe).** A sweep for defects, then a fix for each. What follows is the durable record; the short version is that the two worst were both *silent* — a wrong number shown to students, and a promised feature that did nothing.

**A percentile is about a group, and the group keeps growing.** It was computed once, at submit, and never touched again — so the first student to sit a paper was recorded at the 100th percentile against nobody and kept it forever, while the tenth was ranked against nine people and the hundredth against ninety-nine. Every old score card was quietly measuring a different population from the one it named. `refresh_exam_percentiles` (nightly) re-ranks each cohort through `rerank_exam`: one query out, one `bulk_update` back, because a `calculate_percentile` per row is two COUNTs each and turns a popular paper into an O(n²) sweep. Ties share a rank. Generated drills are skipped — a drill belongs to one student, so ranking it compares them to themselves.

**`is_adaptive` did nothing.** Stored since the first migration, editable in the admin, published by three serializers, read by nothing: an admin could tick "adaptive" on a mock and the paper stayed static. Now `apps/assessments/adaptive.py` implements the real rule — module 1 is the same for everyone, module 2 is chosen from how module 1 went.
- **Variants live on `ExamQuestion.routing`, not on a second `ExamSection`.** A section carries the numbering, the clock, the break and the "Section 2, Module 1" label `examLabels` derives; duplicate the section and every one of those has to learn which twin is real. `standard` is the only value a non-adaptive paper holds — which is why none of this changed an existing exam.
- **The decision is made once, on the advance, and written down** (`SessionModuleRouting`, keeping the accuracy it was made on, exposed read-only in the Django admin so "why did they get the easier module 2" has an answer). Recomputing on read would let `reconcile_session_answers` move a student between forms mid-module.
- **Three places had to learn it**, and the third is the one that would have hurt: serving (one form — and an *unreached* routed module reports one form's SIZE, or the runner tells a student their 22-question module holds 44); answering (belonging to the current section is not enough, since the other form's questions are in that section too); and **grading**, which counts what was shown. Counting both forms marks a student as having omitted a module never selected for them.
- Closed on the way: forward-only stopped a student going *back* but not going too far *forward*. A client could name any later section and land there, skipping whole modules — and on an adaptive paper, choose its own form. An advance must now be to the next section.
- `seed_full_mock` builds the mock adaptively (lower form from the easy end of the bank, upper from the hard end). **Verified over HTTP:** two students, 86% and 18% on module 1, routed to upper and lower — different questions, zero overlap; a submitted paper graded **71** of the template's 108 rows; the unserved form answered "That question is not part of your test."

**Search read the whole table.** Both surfaces were `icontains` — `LIKE '%term%'` over stem and passage, which no index can serve. Postgres now matches a GIN-indexed `tsvector` (`apps/question_bank/search.py`): measured on 20k rows against a selective term, **0.17ms vs 6.4ms**, and the gap widens linearly. Index and query build the vector from one `question_search_vector()` — two definitions mean an index the planner declines to use. SQLite keeps the LIKE path; the migration is `AddIndexPostgresOnly` (`common/migration_ops.py`), which records the index in migration STATE everywhere but emits DDL only on Postgres, so `makemigrations --check` stays clean. A single word matches as a **prefix** (`algeb:*`); anything else goes through `websearch`, which parses the term as data — `query_plan` is a pure function so that decision can be asserted directly. **Gotcha:** no explicit `Coalesce` in the vector. `SearchVector` already coalesces; adding our own needed `output_field=TextField()`, and Django compares field instances by creation counter, so a fresh instance per call made the autodetector see the index change on *every* run.

**The concurrency guards had never run.** The no-double-book partial unique index and the office-hour capacity lock were designed Postgres-only and then never tested, because dev and CI are SQLite. `apps/support/tests/test_concurrency.py` runs them for real — threads with their own connections, released from a barrier (threads spawned in a loop run one after another, which is the interleaving that would *not* catch the bug). Verified by removing the guard: without `select_for_update`, **eight students got into a three-seat office hour**. New `backend-postgres` CI job. On SQLite the race cases skip; the constraint cases still run. Note both booking refusals are legitimate — `slot_unavailable` when the freshness check catches it, `slot_taken` when only the index can.

**Ops.** Nginx `client_max_body_size` was **below** the app's 25 MB ceiling, so a 22 MB upload died at the proxy with a bare 413 and no error envelope. CSP + `X-Frame-Options DENY` added in an **included** file, because nginx's `add_header` does not merge — a location setting any header of its own drops every inherited one, and the two static locations were about to serve assets unprotected. One Celery worker drained all four queues, making the routing decorative (a nightly rollup blocks a verification code while a student stares at a form); email has its own worker now. `/_next/image` was an **open proxy** (`hostname: '**'`) — emptied, and nothing is lost because `next/image` is not imported anywhere. And there were **no backups**, only a runbook line: a `backup` service now dumps on a loop with retention, writing to `.partial` and renaming on success, pruning only after a good dump. Verified against a real cluster including a **restore of all 20,020 rows** — which found a bug in the script itself: `pg_dump | gzip` reports *gzip's* status, and gzip succeeds at compressing a failed dump's empty output.

**Observability.** `/healthz` would have restarted every container during a database blip — the exact failure its docstring says it prevents. `ATOMIC_REQUESTS` is global, so Django opened a connection *before* the view ran; the existing test passed only because it patched the view's own `connection`. Both probes and the new intake are `non_atomic_requests`, pinned by a test that kills the connection at the layer the request wrapper uses. The frontend had **no error tracking at all** — a crash in an exam wrote to a console nobody was watching. `reportClientError` → `POST /api/v1/client-errors/` → logged at ERROR → a Sentry event in the same project (no second DSN, no build plugin, no large dependency). Unauthenticated on purpose, so it is an allowlist rather than a sanitizer, truncated, body-capped and rate limited. Plus `global-error.tsx`, which uses **none** of the providers — `useT` inside the boundary handling a failed `I18nProvider` throws again.

**Localized refusals.** Code errors were English in an Uzbek interface, on the one screen where the message *is* the instruction. The server sends a stable code (`CODE_EXPIRED`, `CODE_INVALID`, …) plus the numbers a sentence needs; `localizeApiError` builds it; unknown codes fall through to the server's English. **Gotcha:** axios camelizes SUCCESS bodies only, so an error envelope arrives as Django wrote it — `parseApiError` now camelizes the extra keys, or every count-bearing sentence would render without its count, silently.

**Verified:** backend **957 pytest on SQLite / 961 on Postgres** (the suite had never run on Postgres before — which is how the `Coalesce` mixed-type error was caught, a failure that could only ever have happened in production) + ruff/black/`makemigrations --check` on both; frontend type-check + lint + **154 vitest** + `next build`. Still not built: the Docker images (no Docker on this machine).*
