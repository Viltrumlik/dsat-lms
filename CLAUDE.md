...
> **Status:** plan LOCKED. **S0 + S1 + S2 SHIPPED** (branch `feat/phase4-teacher-crm`). **S0**: `apps/support`
...
> tab (queue + reply/assign/close-reopen); `support_reply` notifications. Both verified end-to-end in the
> browser + Playwright e2e. **Next up: S4** (Support Session Trigger — the proactive priority). Full
