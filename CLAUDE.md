...
> **Status:** plan LOCKED. **S0 + S1 + S2 + S3 SHIPPED** (branch `feat/phase4-teacher-crm`). **S0**: `apps/support`
...
> lazy bookings-by-status chart) + student summary strip. All verified end-to-end in the browser + Playwright
> e2e. **Next up: S4** (Support Session Trigger — the proactive priority). Full design doc lived in a session
> scratchpad; this section is the durable record.
